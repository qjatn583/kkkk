...
public class MemberJoinAction implements Action{
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
	      ...
	      3.Service 클래스로 Bean타입의 객체를 전달.
	      4.Service 클래스로 부터 처리된 결과를 넘겨 전달받아 성공,실패여부를 판단
	      5.성공하면 로그인 페이지로 넘기고, 실패하면 실패메세지를 띄움.
	      6.결과를 Controller에게 리턴*/
	         
		
		//MemberBean객체를 만들어서 필드값 세팅
		...
		if(joinResult==false){
			...
			System.out.println("검사 joinaction 1");
